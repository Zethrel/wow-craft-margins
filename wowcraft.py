...
def write_addon_prices(path: str, results: list, prices: dict, recipes: list,
                       data_time: int, cfg: dict, batch: int,
    ...
    `data_time` is Blizzard's own Last-Modified for the auction data, NOT the
    `taken_at` day bucket the rest of the pipeline keys history on. That
    distinction is the whole value of the stamp: taken_at is local midnight,
    so passing it made the tooltip report the hours since midnight rather than
    the age of the prices - "4h ago" at four in the morning on data twenty
    minutes old, and "23h ago" at eleven at night. pricecheck.py reads
    last_data_time for the same reason; this had simply never been given it.
...
def publish(out_dir: str, store: Store, cfg: dict, dashboard_path: str,
    ...
    # The published copy is what everyone else's client loads, so it needs the
    # same real timestamp rather than the day bucket.
    try:
        data_time = int(store.get_meta("last_data_time") or taken_at)
    except (TypeError, ValueError):
        data_time = taken_at
    write_addon_prices(lua_path, results, prices, recipes, data_time, cfg,
